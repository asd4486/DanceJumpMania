...
namespace RythmePingPong.Tools
{
	using UnityEngine;
	using System.Collections.Generic;
	...
	public class ParabolaController : MonoBehaviour
	{
		...
		//gizmo
		protected List<ParabolaFly> gizmoList = new List<ParabolaFly>();

		//for play multiple parabola step
		int flyStep;
		//draw
		ParabolaFly nowFly;
		...
		void Update()
		{
			if (!isAnimating || nowFly == null) return;

			if (animationTime < nowFly.GetDuration())
			{
				int parabolaIndexBefore;
				int parabolaIndexAfter;
				nowFly.GetParabolaIndexAtTime(animationTime, out parabolaIndexBefore);
				animationTime += Time.deltaTime;
				nowFly.GetParabolaIndexAtTime(animationTime, out parabolaIndexAfter);

				transform.position = nowFly.GetPositionAtTime(animationTime);
			...
			{
				//auto set next step
				if (flyStep < ParabolaFlys.Count - 1)
				{
					flyStep += 1;
		...
		public void StartParabola()
		{
			...
			animationTime = 0f;
			transform.position = nowFly.Points[0].position;
			isAnimating = true;
		}

		//start at 0 point
		public void StartParabola(int step)
		{
		...
		//start at a specifiq index
		public Vector3 GetHighestPoint(int parabolaIndex)
		{
			var highList = new List<Vector3>();
			foreach (var fly in ParabolaFlys)
				highList.Add(fly.getHighestPoint(parabolaIndex));

			var highest = new Vector3();
			foreach (var h in highList)
				highest = Vector3.Max(highest, h);
		...
		//{
		//	return nowParabolaFly.Points;
		//}

		//public Vector3 GetPositionAtTime(float time)
		//{
		//	return nowParabolaFly.GetPositionAtTime(time);
		//}

		//public float GetSpeed(int index)
		//{
		//	if (index >= ParabolaFlys.Count)
		//		return 0;
		//	return ParabolaFlys[index].GetDuration();
		//}
		...
		public class ParabolaFly
		{
			...
			public void RefreshTransforms(float _duration)
			{
				if (_duration <= 0f)
					_duration = 1f;
				...
				if (Points != null)
				{
					completeDuration = _duration;
					float totalLenght = 0;
					for (int i = 0; i < parabolas.Length; i++)
					{
						if (parabolas[i] == null) parabolas[i] = new Parabola3D();

						parabolas[i].Set(Points[i * 2].position, Points[i * 2 + 1].position, Points[i * 2 + 2].position);
						totalLenght += parabolas[i].Length;
					}

					//create parabolas
					for (int i = 0; i < parabolas.Length; i++)
					{
						var time = parabolas[i].Length == 0 ? 0 : completeDuration * (parabolas[i].Length / totalLenght);
						partDuration[i] = time;
					}
				}
			}
