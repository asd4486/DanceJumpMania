...
namespace RythmePingPong
{
    public class AIParabolaPong : MonoBehaviour
    {
        ...
        public void Init(float duration, float pongHeight)
        {
            ...
            point2.transform.localPosition = new Vector3(xPos / 2, pongHeight * 1.5f, eachTriZPos + eachTriZPos / 2);
            point3.transform.localPosition = point4.transform.localPosition = new Vector3(xPos, pongHeight, eachTriZPos);

            var xPos2 = eachTriZPos / Mathf.Tan(angleC) * (randAngle < 0 ? -1 : 1);
            point5.transform.localPosition = new Vector3(xPos + xPos2 / 2, pongHeight * 0.8f, eachTriZPos / 2);
            point6.transform.localPosition = new Vector3(xPos + xPos2, pongHeight, 0);
